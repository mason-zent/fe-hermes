...
> 진입점은 레포 루트 `AGENTS.md`다. 어느 에이전트(Claude Code·Codex 등)든 거기서 출발해 이 폴더로 온다.
...
검증은 `scripts/verify/<레포>.sh`가 표준이다. 에이전트와 reviewer가 같은 스크립트를 돌려 같은 표 형식으로 보고한다. 실행 전 확인 사항과 결과 해석은 `common/verify.md`.
