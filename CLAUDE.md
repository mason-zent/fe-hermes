...
- **배치는 workspace = 레포 / tab = 작업(워크트리) / pane = 에이전트**다. `delegate.sh` 가 담당 레포 workspace 를 찾아(없으면 만들어) 작업 이름의 탭에 띄운다. 같은 작업의 에이전트 둘은 한 탭에 나란히, 같은 레포의 다른 작업은 다른 탭. `reviewer` 나 단발 조사는 `--here`
...
| `/guide` | 사용·확장 가이드를 터미널에 표시. `/guide 스킬`은 쓸 수 있는 슬래시 커맨드 목록, `/guide pane`은 오른쪽 pane에 선택형 메뉴(스킬 목록·실행 · 라우팅 · git 현황 · 문서), `/guide 열기`는 플레이북 HTML, `/guide 그림`은 **인터랙티브 작업 흐름 다이어그램** 열기 |
