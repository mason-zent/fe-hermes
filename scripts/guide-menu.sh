...
HELPS=(
  ...
  "다이어그램 목록을 브라우저로 연다. 헤르메스 작업 흐름(Plan-First 6단계, 단계별 규칙이 카드에 들어 있다)과 담당 서비스 9개를 네 각도로 본 다이어그램이 있다 — 구조 / 작업 절차 / 요청 흐름 / 화면 상태. 노드를 클릭하면 상세가 뜨고 SRC 배지가 붙은 노드는 실제 파일 경로를 가리킨다. Archify 로 만들며 원본은 docs/diagrams/*.json (브라우저)"
