...
- **작업은 새 작업 브랜치에서 시작한다.** 지금 체크아웃된 브랜치에 그냥 얹지 않는다. 브랜치를 딸 기준은 `hermes.config.json` 의 `prBase`(개발 브랜치)이고, 문서·지식의 기준(`branch`, 운영 반영분)과 다르다. 기본은 **워크트리**로 만들어 메인 체크아웃을 건드리지 않는다 — 레포에 미커밋 변경이 있어도 시작할 수 있고 여러 작업을 동시에 돌릴 수 있다. 미커밋 변경을 stash 하지 않는다(남의 작업일 수 있다). Claude Code 는 `/branch`
